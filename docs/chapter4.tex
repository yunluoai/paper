\chapter{实验结果与性能分析}
本章通过一系列系统性实验对上文研究内容进行全面验证与分析，依次验证GlobalISel在DSP后端生成代码的正确性、本文提出的GlobalISel分阶段优化策略的有效性，并开展其与传统指令选择方案的综合性能对比。


%*********************************************************************
% 4.1 GlobalISel代码生成正确性验证
%*********************************************************************
...
本节旨在验证DSP后端基本指令的正确性，重点关注算术运算指令和内存访问指令的匹配和选择结果。为此本节设计了一个示例程序一，这个示例程序包含函数参数传递、内存读写操作和整数加法与乘法运算等操作，能够覆盖DSP后端常见的指令选择路径。由于LLVM IR具有良好的目标无关性，且从C语言到LLVM IR的转换过程并非本文关注重点，因此本节及后续章节在分析过程中直接展示从LLVM IR到目标机器码的转换流程。


\subsubsection{1.LLVM IR输入}
示例程序一的LLVM IR如下所示，该示例程序包含了基本的算术与访存操作，后文将以此为基础分析指令选择过程中中间表示的演化。
...
在机器指令选择阶段，GMIR被转换为DSP架构的目标机器指令。如\ref{fig:case_1_instr_select_dump}所示，从中间结果可以观察到，除了COPY指令外的所有指令都被正确地映射到DSP后端机器指令，COPY指令需要在后续阶段中单独处理。
...
在完成寄存器分配以及指令调度等后续后端处理流程后，编译器最终生成了符合DSP指令集规范的目标代码。如图\ref{fig:case_1_final_dump}所示，生成的目标代码在指令顺序、操作数选择以及控制流结构等方面均与原始程序语义保持一致。这表明在该示例程序中，基于GlobalISel框架的代码生成流程能够在DSP后端正确运行，实现了从高级LLVM IR到目标平台可执行机器指令的精准映射。
...
比较与条件分支是程序控制流构建的基础。在LLVM IR中，比较与条件分支通过icmp与br组合表达。为验证DSP后端对该类控制流指令的处理正确性，本节构建了一个同时包含比较、条件分支及无条件跳转的示例程序二，其LLVM IR如下所示。
...
函数调用是一类重要的控制流操作，其正确性依赖于调用约定、参数传递机制以及返回值处理等多个环节。为验证DSP后端在GlobalISel框架下对函数调用指令的处理正确性，构造了一个简单的示例程序三，其中main函数调用前文定义的testBranch函数，其LLVM IR如下所示。
...
\begin{figure}[htbp]
	...
	\caption{示例程序三的最终阶段Dump图}
	\label{fig:case_4_final_dump}
\end{figure}


%*********************************************************************
% 4.2 GlobalISel优化策略有效性验证
%*********************************************************************
\section{GlobalISel优化策略有效性验证}
为验证本文所提出优化策略的有效性，本节通过一系列实验分别对两个阶段的优化效果进行验证。首先进行基于典型样例的针对性验证，构造一个符合优化触发条件的典型测试程序，通过观察优化前后的中间表示变化，判断优化是否正确触发并产生预期结果。然后在Embench基准测试集进行系统性量化评估。实验通过对比优化开启与优化关闭两种配置下生成的代码，从代码体积与执行周期两个维度进行量化分析，从而系统评估优化对性能产生的实际影响。
...
为验证合法化前优化是否有效，本节以内存操作优化为例设计了示例程序四，程序包含一个内存拷贝函数，用于从源地址拷贝16字节的32位整型数据到目标地址，其LLVM IR如下所示。
...
从图\ref{fig:pre_optimization_code_size}可以观察到，除crc32样例的代码体积缩减率为负值外，其余所有测试样例的缩减率均为正值，说明本文提出的优化方法能够使绝大多数程序的代码体积得到有效减小。其中，crc32样例优化效果相对较弱，主要原因如下：

\begin{itemize}
	\item
	crc32的核心计算逻辑为循环移位与异或折叠的位运算算法，其程序特征天然限制了编译器的优化空间。
	
	\item
	算法具有较强的串行数据依赖，每一步计算均依赖前序结果，难以挖掘指令级并行性与向量化潜力。
	
	\item
	程序控制流简单、循环体短小，编译器可进行优化的空间有限。

	\item 
	核心位运算逻辑已接近最优序列，指令选择与调度优化带来的收益被显著压缩。
	
\end{itemize}

因此，无论采用 SelectionDAGISel 还是 GlobalISel，二者生成的指令序列差异较小，优化提升不明显。
...
从图\ref{fig:pre_optimization_cycle}可以观察到，所有样例的执行周期缩减率均为正值，说明执行效率均得到提升。值得注意的是，尽管crc32样例的代码体积略有增大，但其执行周期却得到了明显降低。出现这一现象的主要原因在于，GlobalISel的指令选择与指令调度优化机制与crc32自身的位运算特征高度匹配。crc32的核心由大量串行位操作构成，其性能瓶颈并非指令数量多少，而是关键路径延迟的长短。GlobalISel在指令选择阶段能够针对移位、异或、位域操作等核心计算生成延迟更低、可单周期执行的专用指令，而非简单的通用指令序列拼接。尽管为实现低延迟指令会引入少量辅助指令并导致代码体积略有上升，但关键路径长度被显著缩短，最终使整体执行周期明显下降。同时，crc32控制流结构简单、不存在复杂分支干扰，后端优化能够稳定发挥指令调度的优势。因此，crc32以极小的空间代价换取了明显的性能提升，属于典型的以空间换时间优化，体现了GlobalISel对位运算密集程序的优化效果。
...
\subsubsection{1.典型样例验证}
为验证合法化后优化是否有效，本节以常量乘法强度削弱为例设计了示例程序五，程序包含一个32位整型乘法指令，其LLVM IR如下所示。
...
图\ref{fig:case_6_before_dump}与图\ref{fig:case_6_after_dump}展示了优化前后的GMIR对比，从图中可以看出优化前的G\_MUL指令在优化后被成功替换为G\_SHL与G\_ADD指令的组合。此转换证明了常量乘法强度削弱优化在合法化后阶段被正确触发并应用。


\subsubsection{2.系统性量化评估}
以优化等级O2为例并选取与4.2.1节相同的10个样例，在开优化和不开优化两种条件下，对这些样例的代码体积和执行周期进行统计并通过可视化方式呈现优化前后的量化对比结果。
...
从图\ref{fig:post_optimization_cycle}可以观察到，大部分样例的执行周期均获得正向缩减，其中mont64样例的缩减效果最为突出，执行周期缩减率高达18.3536\%；edn、md5sum、picojpeg等样例也实现了1\%至3\%的性能提升；仅huffbench与st样例的执行周期数出现了小幅增加。mont64样例优化收益显著，源于其作为大数模乘与Montgomery乘法计算内核，具备典型的计算密集特征：算术运算占比高、指令级并行潜力大，循环体内存在大量独立乘加操作，且控制流规整无复杂分支，便于GlobalISel完成指令选择、寄存器分配与指令调度的协同优化。同时，较多的中间运算结果可充分发挥GlobalISel在多寄存器组管理、值传播与合法化处理上的架构优势，从而获得远高于其他用例的性能收益。
...
以上实验结果表明：本文实现的面向DSP后端的合法化后优化在大多数场景下都能有效减少执行周期并提高执行效率。从优化效果上来看，合法化后优化并不像合法化前优化那样普遍提升效率，这是因为合法化后优化针对性更强，仅在特定指令组合模式下才能发挥作用。


%*********************************************************************
% 4.3 GlobalISel综合性能对比分析
%*********************************************************************
\section{GlobalISel综合性能对比分析}
上一节已通过对比实验分别验证了合法化前、后优化策略的有效性，为全面评估GlobalISel框架相较于传统指令选择方案的综合性能优势，本节从代码大小、执行周期以及编译时间三个维度出发，对基于DAG的指令选择、未优化的全局指令选择以及优化后的全局指令选择来进行全方位的对比。
...
本节选取Embench作为测试集，对每个基准程序分别在四个优化等级下进行编译，并统计生成的目标代码text段大小。对于同一优化等级下的同一测试样例，取单次编译结果作为该样例的代码体积。随后，把测试集中所有基准程序的结果进行平均计算，得到各个优化等级下的平均代码段体积，用于不同指令选择方案之间的对比分析。
...
实验结果表明，相较于未优化的GlobalISel实现，引入针对DSP架构特点的指令选择与指令组合优化后，编译器在各优化等级下均生成了更紧凑的目标代码。这表明本文提出的优化策略不仅在性能层面有效，还在代码体积这一关键指标上取得效果，为后续嵌入式场景的实际部署提供了有力支撑。
...
编译时间的测量易受到操作系统调度、CPU频率波动、磁盘I/O延迟、内存缓存状态等环境噪声的影响。单次测量结果往往存在偶然性，无法准确反映编译器后端的真实执行开销。为降低上述干扰因素对实验结果的影响，提升测量结果的稳定性、准确性与可重复性，实验采用多次重复测量+去极值平均的统计方法。具体而言，在保持实验环境与编译配置完全一致的条件下，对每个测试样例独立执行10次完整的编译流程。在结果统计阶段，剔除2个最大值与2个最小值，再对剩余6组有效数据取算术平均值，以此作为该样例在对应优化等级下的编译时间。最后对Embench测试集中22个基准程序的结果汇总并进一步取平均值，得到不同优化等级下的平均编译时间，以此用于不同指令选择方案之间的对比分析。
...
从图\ref{fig:compile_time_contrast}中可以观察到，相比于SelectionDAGISel，GlobalISel在4个优化等级下的平均编译时间均更短，尤其是在O0优化等级下，平均编译时间只有SelectionDAGISel的一半。这个现象表明在中端优化尚未充分展开的情况下，指令选择框架自身的设计差异就已经对后端编译效率产生了影响。


\subsubsection{4.实验结论}
从实验结果可以观察到，在编译时间上GlobalISel相比SelectionDAG具备明显优势，且该优势在各优化等级下均保持一致。这说明该优势并非源于特定的优化配置，而是来自指令选择框架本身在流程设计和中间表示处理上的效率优化，这一特性使GlobalISel更适配频繁编译、快速迭代的开发场景。
...
随后针对本文提出的两阶段优化策略，编写典型样例来进行有效性的验证，之后基于Embench测试集开展系统性量化评估，实验结果表明两阶段优化策略在多数场景都能有效减少代码体积和执行周期。

\par

最后对代码体积、执行周期以及编译时间等关键性能指标进行了全面的对比分析。实验结果表明，优化后的GlobalISel在各关键环节均能保持原始程序语义与数据依赖关系，其生成的目标代码符合DSP指令集规范与控制流逻辑，并且在性能上有明显的提升。
