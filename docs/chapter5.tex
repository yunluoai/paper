...
% 5.2 全局指令选择优化有效性验证
%*********************************************************************
\section{全局指令选择优化有效性验证}
为验证本文所提出优化策略的有效性，本节分别选取合法化前与合法化后两个阶段的典型优化进行测试与分析，通过对中间表示的对比验证优化是否按预期触发并产生正确结果。


\subsubsection{1.合法化前优化策略的验证}
为验证合法化前的优化效果，本节以内存操作优化为例设计了示例程序五，其LLVM IR如代码块\ref{lst:case_5_ir}所示。
...
在未启用优化的编译流程中，该调用会保持为一次函数调用。在启用了本文实现的合法化前优化后，编译器应在GMIR生成阶段将此内存拷贝调用识别并转换为等价的基础内存操作指令序列。图\ref{fig:case_5_before_dump}与图\ref{fig:case_5_after_dump}分别展示了优化前后的中间表示对比。
...
对比两图可以看出，优化前的memcpy指令在优化后被成功替换为一组G\_LOAD与G\_STORE指令序列，完成了从内置函数到基础内存操作的转换，证明了该合法化前优化策略的有效性。


\subsubsection{2.合法化后优化策略的验证}
为验证合法化后的优化效果，本节以常量乘法强度削弱为例设计了示例程序六，其LLVM IR如代码块 \ref{lst:case_6_ir} 所示。
